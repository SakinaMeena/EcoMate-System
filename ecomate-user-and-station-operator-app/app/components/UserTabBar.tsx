...
type TabItem = {
  key: "userHome" | "scan" | "profile";
  label: string;
};

export default function UserTabBar({ state, navigation }: BottomTabBarProps) {
  const router = useRouter();

  const tabs: TabItem[] = [
    { key: "userHome", label: "Home" },
    { key: "scan", label: "Scan" },
  ...
  const goTo = (name: TabItem["key"]) => {
    if (name === "profile") {
      ...
      router.push("/(main)/(gamification)/profile");
      return;
    }
